...
function showImg() {
  picArray.forEach((pic) => {
    ...
    imgElem.src = pic.image.medium;
    imgElem.alt = pic.title;

    const figcaptionElem = document.createElement("figcaption");
    figcaptionElem.textContent = pic.caption;

    const pElem = document.createElement("p");
    pElem.textContent = pic.description;
    ...
    const picturesElem = document.querySelector("#pictures");
    picturesElem.appendChild(articleElem);
  });
}

function showModal(e) {
  const dialogImgElem = document.querySelector("dialog img");
  const dialogElem = document.querySelector("dialog");
  const imgID =
    parseInt(
      e.target.closest("article").querySelector("h2").textContent.slice(-1),
    ) - 1;
  dialogImgElem.src = picArray[imgID].image.large;
  dialogImgElem.alt = picArray[imgID].title;
  dialogElem.show();
}
...
(() => {
  showImg();
  document.querySelector("#pictures").addEventListener("click", showModal);
  document.querySelector("dialog span").addEventListener("click", close);
})();
